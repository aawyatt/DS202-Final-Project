...
```{r}
ufo1 %>% ggplot(aes(x = long.border, y = lat.border)) + geom_path(aes(group = group)) +geom_point(aes(x = longitude, y = latitude))
```

### Histogram (datetime)

```{r}
ufo1 %>% ggplot(aes(x = datetime)) + geom_histogram()
```
...
ufo5 = ufo5[1:5, ]
ufo1 %>% filter(shape %in% ufo5$shape) %>% ggplot() + geom_mosaic(aes(x = shape, fill = factor(region), weight = 1))
```
...
```{r}
ufo5 %>% ggplot(aes(x = shape, y = n)) + geom_boxplot()
```
